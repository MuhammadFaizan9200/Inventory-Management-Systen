...
session_start();
    ...
    if($_POST){
    	...
    	if($_GET['id']){
    		$id = $_GET['id'];
    		$query = "update `company` set `company_name` ='$company_name' ,`status` ='$status' where id = '$id'";
    	}else{
...
<head>
    <title>Companies | Ahmed Traders</title>
        ...
        <div class="layout-inner">
            ...
            <!-- Modal template end-->
                   ...
                   <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0">New Company</h4>
                        ...
                        <div class="text-muted small mt-0 mb-4 d-block breadcrumb">
                        	<button style="width: 25%" type="submit" class="btn btn-success btn btn-block add_company"  role="button">Add Company</button>
                        </div>
                       
                        <!-- / Filters -->

                        <div class="card">
                             <h3 class="card-header bg-primary text-white" align="Center"><i class="oi oi-spreadsheet" style="font-size:35px; "></i> &nbsp; Company List</h3>
                            ...
                            <table class="datatables-demo table table-striped table-bordered" id="editable-table">
                                ...
                                <tbody>
                                    ...
                                    echo"<tr> 
                                          <td>$count</td>
                                           <td>$company_name<input type='hidden' value='".$id."' class='id'><input type='hidden' value='".$company_name."' class='company_name'></td>
...
<script>
    ...
    $('body').on('click','.edit_company', function(event){
    	var company_name = $(this).closest('tr').find('.company_name').val();
    	var id = $(this).closest('tr').find('.id').val();
            
          $('.modal-dialog').html(' <form class="modal-content" method="post" action="companies.php?id='+id+'"><div class="modal-body form-horizontal"><div class="form-row"><div class="form-group col mb-4">\ <label class="form-label">Company Name</label><input required type="text"  class="form-control" placeholder="Company Name" value="'+company_name+'" name="company_name"><div class="clearfix"></div> </div></div><div class="form-row"><div class="form-group col mb-4">\ <label class="form-label">Company Hide</label><br><input id="toggle-one" value="1" type="checkbox" name="status"><div class="clearfix"></div> </div></div><div class="form-row" style="float: right;"><div class="form-group col mb-4"><button type="button" class="btn btn-default" data-dismiss="modal">Close</button>   <button type="submit" class="btn btn-primary">Save</button></div></div></form>');
         $('#modals-default').modal('show');
          $(function() {
    ...
    $("table").on("click", ".delete", function (event) {
                if (confirm("Are you sure you want to delete this Company?")) {
                    var ID = this.id;
                     var $this = $(this); 
                    $.get("delete.php?company_id="+ID, function (data, status) {
                        if (status == 'success') {
                            var Row = $this.closest('tr');
                              var tr = $this.parents('tr');
                              var nRow = Row[0];
                            showmessage('warning','Company Delete Successfully','Success');  
