<?php
      ...
      require("fpdf/fpdf.php");
        ...
        $get_company_name = @$_GET['get_company_name'];
...
$fpdf = new FPDF("p",'mm','A4');
    ...
    $fpdf->Cell(55,7,'Product Name', 'T B', 0, 'L');
    $fpdf->Cell(25,7,'Company Name', 'T B', 0, 'L');
    $fpdf->Cell(10,7,'Qty', 'T B', 0, 'L');
    $fpdf->Cell(20,7,'Sale Amount', 'T B', 0, 'L');
    $fpdf->Cell(10,7,'Dis', 'T B', 0, 'L');
    $fpdf->Cell(20,7,'Total', 'T B', 1, 'L');
  

   $fetch_query = "SELECT sid.* ,s.invoiceNumber,s.invoiceDate ,(SELECT product_name from products where id = sid.insert_productName) as productName ,(SELECT company_name from company where id = sid.`company_id`) as companyName ,(SELECT totalNetAmount from sale_invoice_amount_details where sale_invoice_id = s.id) as totalNetAmount FROM `sale_invoice_details` sid , sale_invoice s WHERE s.id = sid.`sale_invoice_id` $companyVariable $dateBtween $dateData order by s.invoiceDate asc , sid.id desc";
          ...
          while($rows = $sth->fetch(PDO::FETCH_ASSOC)){
              ...
              $grand_net_amount += $totalNetAmount;
                ...
                $fpdf->Cell(20,7,''.$invoiceNumber.' ', 'B R', 0, 'L',$fill);
                 $fpdf->Cell(55,7,''.$productName.' ','B R', 0, 'L',$fill);
                $fpdf->Cell(25,7,''.$companyName.' ', 'B R', 0, 'L',$fill);
                $fpdf->Cell(10,7,''.$total_quantity.' ', 'B R', 0, 'L',$fill);
               ...
               $fpdf->SetFont('Times','B',8);
               $fpdf->Cell(135,7,'Grand Total', 'B R', 0, 'L');
